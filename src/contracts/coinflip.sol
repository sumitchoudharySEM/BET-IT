...
contract CoinFlip {
    ...
    struct Game  {
        ...
        string GameResult; // win for a win, loose for a loose
    }
    ...
    function StartGame(uint256 StackAmountVal, bool ChosenCoinSideVal) public payable returns(string memory) {
        ...
        if (result) {
            Games[GameNumber].GameResult = "win";
            ...
            Games[GameNumber].GameResult = "loos";
            Games[GameNumber].GameState = 2;
            GameNumber ++;  
            return "loos";
        }
